...
def export_csv(filename, devices, delimiter=";"):
    ...
    with open(outfile, mode='w') as csv_file:
        ...
        w.writerow(['AP-MAC', 'SSID', 'Encryption','Client-MAC'])
        for dev in devices:
...
def gen_aplist():
    ...
    parser.add_argument("--ssid", action="store", dest="ssid",
                        help='Only plot networks which match the SSID (or SSID regex)')
    parser.add_argument("--exclude-ssid", action="store", dest="excludessid",
                        help='Only plot networks which match the SSID (or SSID regex)')
    ...
    parser.add_argument("--encryption", action="store", dest="encryption", default=None,
                        help="show only networks with given encryption type")
